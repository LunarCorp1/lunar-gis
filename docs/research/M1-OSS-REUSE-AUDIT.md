...
| **D. kodeezabdullah/qgis-ai-agent** | Plugin (`qgis_ai_agent/`) | **Not declared** (repo no `LICENSE` file, plugins.qgis.org shows no license field, 0.1.2 experimental) → treat as **All Rights Reserved** | Claims 3.0+ (not 4-specific, `QWebEngineView` dock) | OpenRouter only, free `kimi-k2.6`, `gpt-oss-120b/20b`, `llama-3.3-70b` fallback chain | 50+ tools **plus** `self-executing PyQGIS code` fallback for uncovered ops | `QWebEngineView` chat → `Processing` + `run_pyqgis` self-exec | **Self-executing fallback is arbitrary Python by default** — violates `AGENTS.md:4`; OpenRouter API key via Settings `OPENROUTER_API_KEY` (user pastes, not hard-coded) | Low-maturity 0.1.2 2026-06-08 1,895 downloads, single author, 22 open issues? Actually 0, recent | Unknown (no `pyproject` parsed) | **REJECT** (for runtime) / **REFERENCE-ONLY** for 50-tool surface idea | **MEDIUM** |
...
| **J. QuickOSM (`3liz/QuickOSM`) / OSMDownloader** | Plugins | **GPL-2.0+** (`3liz/QuickOSM` GPL-2.0, `lcoandrade/OSMDownloader` GPL-2.0) | Yes `3.28+` and QGIS 4 (QuickOSM actively maintained) | No AI (manual key/value) | Overpass API key/value, area/extent, OSM file open with `osmconf` | `Overpass` via `Ogr` parser, `QuickOSM` processes allow model building | Bounded Overpass, no AI exec | **High** QuickOSM 400k+ downloads, mature | `qgis` + `requests` | **ADAPT** pattern (bounded Overpass) | **HIGH** |
...
*Source:* `https://github.com/kodeezabdullah/qgis-ai-agent` no `LICENSE` file (All Rights Reserved), `plugins.qgis.org/plugins/qgis_ai_assistant` `Author kodeezabdullah` 22 downloads. Claims `>50 built-in QGIS operations` `vector/raster NDVI/NDWI/hillshade/slope` + `self-executing PyQGIS code fallback` + `QWebEngineView` dock.

*License:* **Missing LICENSE → REJECT** for code reuse (`AGENTS.md:12` “Do not copy without reviewing license”): cannot treat as `MIT` without file. If we treat as `All Rights Reserved`, only `REFERENCE-ONLY` for idea (50-tool surface, fallback chain `kimi-k2.6` → `gpt-oss-120b` → `llama-3.3-70b`).
...
*License:* **REJECT** — cannot verify `LICENSE` → treat as All Rights Reserved per `AGENTS.md:12`.
...
* **QuickOSM (`3liz/QuickOSM`) / OSMDownloader** — `QGIS plugin to fetch OSM data with Overpass API` `plugins.qgis.org/plugins/QuickOSM` 400k+ downloads **GPL-2.0** (`3liz/QuickOSM` GPL-2.0) + `lcoandrade/OSMDownloader` GPL-2.0 1.1.2 (rectangle selection). **ADAPT** bounded Overpass pattern, GPL compatible, mature.

* **pystac / PySTAC** — For `M4` STAC, mature `PySTAC` `BSD-3` + `planetary-computer` SDK `MIT` are **ADAPT** via `PROVIDER_CONTRACT.md` `search/get_metadata/download` for `AWS Earth Search` & `Microsoft Planetary Computer` (seen in Geo Knowledge AI) — not from chat-agent repos.
...
| `kodeezabdullah/qgis-ai-agent` | **Missing** → All Rights Reserved | No license field | Unknown | **Incompatible** to copy — treat as ARR, not MIT | **Do not** add as dep | **Do not copy** |
...
| `kodeezabdullah/qgis-ai-agent` | **REJECT** (runtime) / **REFERENCE-ONLY** (50-tool surface idea) | **MEDIUM** | Missing `LICENSE` → All Rights Reserved (`AGENTS.md:12`), self-executing fallback by default violates `AGENTS.md:4`, `QWebEngineView` not Qt6-clean, `requires-python` not 4-specific | `github.com/kodeezabdullah/qgis-ai-agent` no `LICENSE`, `plugins.qgis.org` no license field |
...
| `3liz/QuickOSM` / `OSMDownloader` | **ADAPT pattern** | **HIGH** | Mature `400k+` downloads `GPL-2.0` bounded Overpass `key/value` + `osmconf` parser, `Processing` model building | `3liz/QuickOSM` GPL-2.0, `plugins.qgis.org/plugins/QuickOSM` |
| `microsoft/GeoFaham` `PySTAC` etc. | **REFERENCE-ONLY** | **MEDIUM** | Heavy `AutoGen` `PostGIS` vs Lunar QGIS desktop; `PySTAC` `BSD-3` `Planetary Computer` `MIT` are **ADAPT** for `M4` STAC via `PROVIDER_CONTRACT.md` `search/get_metadata/download` | `PySTAC` BSD-3, `Microsoft/GeoFaham` MIT? |
...
* **Data providers to eventually ADAPT (not INTEGRATE) behind `PROVIDER_CONTRACT.md`:**
  * **PySTAC** `BSD-3` (`pystac`) + **`planetary-computer` SDK** `MIT` (`microsoft/planetary-computer`) for `search`/`get_metadata`/`get_assets`/`download`/`attribution`/`license` for `AWS Earth Search` & `Microsoft Planetary Computer` (seen in `geo-knowledge-ai`). Reuse `PySTAC` as **optional** dep, not copy.
  * **pystac-client** `BSD-3` for STAC search pagination.
...
* **License missing repos:** `qgis-ai-agent`, `AgenticGIS`, `geo-knowledge-ai` missing `LICENSE` → treat as `All Rights Reserved`; cannot copy even MIT-like snippets without file.
...
| **qgis-ai-agent** | **REJECT** dep / **REFERENCE-ONLY** idea | **MEDIUM** | Missing LICENSE, self-exec by default. |
| **geogenie** | **REJECT** dep / **REFERENCE-ONLY** pipeline export | **MEDIUM** | GPL-2.0 compatible but 0★, satellite auto-pick not Processing. |
| **geo-knowledge-ai** | **REJECT** | **LOW** | Missing LICENSE, unknown security. |
...
| **PySTAC / Planetary Computer** | **ADAPT** (optional dep, BSD-3/MIT) | **HIGH** | For `M4` STAC via `PROVIDER_CONTRACT.md`. |
